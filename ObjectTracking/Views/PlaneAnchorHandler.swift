...
class PlaneAnchorHandler {
    ...
    func process(_ anchorUpdate: AnchorUpdate<PlaneAnchor>) async {
        ...
        // Only proceed if the detected plane is classified as a table
        ...
        if let meshResource {
            // Set a visible material (e.g., light gray) for the plane
            let planeMaterial = SimpleMaterial(color: .gray, roughness: 0.9, isMetallic: false)
            entity.components.set(ModelComponent(mesh: meshResource, materials: [planeMaterial]))
